...
describe('VestingCreator', function () {
  it('VestingCreator must be deployed correctly', async () => {
    ...
    vestingCreator = <VestingCreator>await deployer.contractDeploy('test/VestingCreator', []);
    dkToken = <DuelistKingToken>await deployer.contractDeploy('test/DuelistKingToken', [], vestingCreator.address);
  ...
  it('beneficiary of vesting should not able to withdraw after vesting less than 1 day', async () => {
    let error = false;
    try {
      await timeTravel(dayToSec(30.5));
      await myVestingContract.withdraw();
    } catch (e: any) {
      console.log(`\t${e.message}`);
      error = true;
