...
contract DAO is User, IDAO {
  ...
  function createProposal(Proposal memory newProposal) external override returns (uint256) {
    uint256 votePower = IDAOToken(registry.getAddress(domain, bytes32('DAOToken'))).votePower(msg.sender);
  ...
  function execute(uint256 proposalId) external override returns (bool) {
    ...
    int256 threshhold = int256(IDAOToken(registry.getAddress(domain, bytes32('DAOToken'))).totalSupply() / 2);
